    ...
    interpolation_map = {
        {"linear",
         [](const std::vector<std::vector<double>>& dataset) {
             return std::make_unique<Linear2DInterpolation>(dataset);
         }},
        {"spline",
         [](const std::vector<std::vector<double>>& dataset) {
             return std::make_unique<Spline2DInterpolation>(dataset);
         }},
        {"poly",
         [](const std::vector<std::vector<double>>& dataset) {
             return std::make_unique<Polynomial2DInterpolation>(dataset);
         }},
        {"akima", [](const std::vector<std::vector<double>>& dataset) {
             return std::make_unique<Akima2DInterpolation>(dataset);
         }}};
